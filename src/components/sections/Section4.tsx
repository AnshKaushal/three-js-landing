...
const Section4 = forwardRef<HTMLDivElement>((_props, ref) => {
  return (
    <div ref={ref} className="max-w-xl">
      ...
      <h2 className="font-heading text-[clamp(2.5rem,8vw,6rem)] font-bold leading-none tracking-tighter text-white">
        CLIENTS
      </h2>
      ...
      <div className="mt-8 grid grid-cols-2 gap-3 sm:grid-cols-4">
        {partners.map((name) => (
          <div
            key={name}
            className="flex items-center gap-2 rounded border border-white/50 px-3 py-2 text-[11px] font-medium tracking-wide text-white/90 uppercase"
          >
            <IconStar className="size-3 shrink-0 text-white/20" />
            {name}
          </div>
        ))}
      </div>
